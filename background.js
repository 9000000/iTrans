...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  if (request.action === "translate") {
  ...
  } else if (request.action === "setPopupPos") {
    chrome.storage.sync
      .set({ popupPos: request.data })
      .then(() => {
    ...
    return true;
  } else if (request.action === "getPopupPos") {
    chrome.storage.sync
      .get(["popupPos"])
      .then((data) => {
        sendResponse({ popupPos: data.popupPos });
      })
      .catch((error) => {
        sendResponse({ popupPos: null, error: error.message }); // Send null or handle error
      });
    return true;
  }
});
...
async function translateText(text) {
  try {
    ...
    for (let attempt = 0; attempt < apiKeys.length; attempt++) {
      ...
      try {
        const translation = await callGeminiAPI(apiKey, text, targetLang);

      ...
      chrome.runtime.sendMessage(
        ...
        (response) => {
          if (chrome.runtime.lastError) {
            console.error(
              "Error setting popup position:",
              chrome.runtime.lastError.message,
            );
          } else if (!response || !response.success) {
            console.warn("Failed to set popup position:", response);
          }
        },
      );
    } else {
      console.warn(
        "Chrome runtime is not available, cannot save popup position.",
      );
    }
  }
...
async function translateSelectedText() {
  ...
  try {
    ...
    if (typeof chrome?.runtime?.sendMessage !== "function") {
      console.error(
        "Error retrieving popup position: Chrome runtime message sending is not available. Reload the page.",
      );
      // saved will remain null, leading to default positioning.
      // The outer catch block for "Error loading popup position" will still log this,
      // but the TypeError should be prevented.
    } else {
      const response = await chrome.runtime.sendMessage({
        action: "getPopupPos",
      });

      // Check for runtime error after sendMessage
      if (chrome.runtime.lastError) {
        console.error(
          "Error retrieving popup position:",
          chrome.runtime.lastError.message,
        );
      } else {
        saved = response ? response.popupPos : null;
      }
    }
  ...
  try {
    let result;
    if (typeof chrome?.runtime?.sendMessage !== "function") {
      // If sendMessage is not available, it means the extension context is invalidated.
      // We throw a specific error that our outer catch block already knows how to handle.
    ...
    try {
      ...
      // Re-throw or handle as a translation error
      // This catch is mainly for errors like "Extension context invalidated"
      // which we already handle below, but for other sendMessage failures too.
    ...
    if (result && result.success) {
      ...
      // Fallback for unexpected result or undefined result
      translatedDiv.innerHTML = `<div class=\"ai-error\">❌ Lỗi dịch: Không nhận được phản hồi hợp lệ từ tiện ích. Vui lòng tải lại trang.</div>`;
    }
  } catch (error) {
    if (
      error.message &&
      ...
      translatedDiv.innerHTML = `<div class=\"ai-error\">❌ Tiện ích mở rộng đã được tải lại. Vui lòng tải lại trang để tiếp tục dịch.</div>`;
    } else {
